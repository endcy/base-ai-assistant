...
import com.endcy.ai.repository.domain.vector.VectorDocument;
    ...
    public static List<KnowledgeDocumentMatchItem> documentConvertRelated(List<Document> relatedDocuments) {
        ...
        List<KnowledgeDocumentMatchItem> list = relatedDocuments.stream()
                                                                .map(document -> {
                                                                    ...
                                                                    Object docId = document.getMetadata().get("id");
                                                                    return KnowledgeDocumentMatchItem.builder()
                                                                                                     .docId(docId == null ? 0L : Long.parseLong(docId.toString()))
        ...
        for (KnowledgeDocumentMatchItem item : list) {
            boolean isExistId = resultItems.stream().anyMatch(resultItem -> resultItem.getDocId().equals(item.getDocId()));
            if (!isExistId) {
