...
import com.endcy.ai.rpc.enums.DocumentActionType;
import com.endcy.service.common.annotation.LogReqRes;
import com.endcy.service.common.constant.GlobalConstant;
...
public class DocumentFeignProcessor {
    ...
    public CommonResMsgDTO<KnowledgeDocumentStatusItem> modify(@Validated @RequestBody KnowledgeDocumentParam document) {
        ...
        ret.setDocId(document.getDocId());
        ret.setLoaded(false);
        ...
        if (res.getId() != null) {
            KnowledgeDocumentDTO fresh = knowledgeDocumentService.getById(res.getId());
            if (fresh != null) {
                ret.setDocId(fresh.getId());
                ret.setLoaded(fresh.getLoaded());
                ret.setEnabled(fresh.getEnabled());
            }
        }
    ...
    public CommonResMsgDTO<KnowledgeDocumentStatusRet> action(@Validated @RequestBody KnowledgeDocumentActionParam query) {
        ...
        switch (actionType) {
            case DISABLED -> {
                ...
                if (CollUtil.isNotEmpty(query.getDocIds())) {
                    knowledgeDocumentService.updateDocumentEnabledStatus(query.getDocIds(), false);
                }
            }
            case ENABLED -> {
                if (CollUtil.isNotEmpty(query.getDocIds())) {
                    knowledgeDocumentService.updateDocumentEnabledStatus(query.getDocIds(), true);
                    needRefresh = true;
                }
            }
            case REFRESH -> {
            ...
            case DELETE -> {
                //更新知识库，同时清除向量库对应文档
                if (CollUtil.isNotEmpty(query.getDocIds())) {
                    knowledgeDocumentService.removeByIds(CollUtil.newHashSet(query.getDocIds()));
                }
            }
        }
        //刷新向量
        ...
        //如果指定了文档id列表，按id过滤
        if (CollUtil.isNotEmpty(query.getDocIds())) {
            HashSet<Long> idSet = CollUtil.newHashSet(query.getDocIds());
     ...
     * 构建文档查询参数
     */
    private KnowledgeDocumentQueryParam buildQueryParam(String scopeType, String businessType) {
        KnowledgeDocumentQueryParam queryParam = new KnowledgeDocumentQueryParam();
