...
public interface EnergyAiConstant {
    ...
    String DEFAULT_SCOPE_TYPE = "用户客服";
    int RAG_QUERY_TYPE = 1;

    /**
     * 多媒体分析系统提示词（多模态模型用，提取媒体文件中与用户问题相关的关键信息）
     */
    String MEDIA_ANALYSIS_SYSTEM_PROMPT = """
            你是一个专业的多媒体内容分析助手。你的任务是分析用户提供的媒体文件（图片、音频如mp3、视频、文档等），提取出与用户问题相关的关键信息，生成简洁准确的文本描述。
            
            要求：
            1. 重点关注与用户问题相关的信息，优先提取故障码、数值、错误提示、设备型号、时间等关键内容
            2. 如果图片、音频、视频或文档中包含文字、语音或字幕，请转录并提取其文字内容
            3. 描述要简洁、准确、有条理，描述长度控制在500字以内，根据侧重的信息内容可以精简
            4. 如果无法识别内容，如实说明
            5. 直接输出描述内容，不要添加"图片显示"、"音频内容"等前缀
            """;

    /**
     * 多媒体分析用户提示词（%s 为用户原始问题）
     */
    String MEDIA_ANALYSIS_USER_PROMPT = """
            用户的问题：%s
            
            请分析提供的媒体文件，提取与用户问题相关的关键信息。
            """;
